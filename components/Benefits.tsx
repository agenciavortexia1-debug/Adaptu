...
const Benefits: React.FC = () => {
  ...
  return (
    <section id="benefits" className="py-24 bg-slate-950 scroll-mt-16 text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl font-extrabold text-white">Por que escolher o personalizado?</h2>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {benefits.map((benefit, idx) => (
            <div key={idx} className="p-8 rounded-md bg-slate-900 border border-slate-800 shadow-sm hover:shadow-emerald-500/10 hover:border-emerald-500/30 transition-all duration-300">
              <div className="w-12 h-12 bg-emerald-900/30 text-emerald-400 rounded-sm flex items-center justify-center mb-6">
                {benefit.icon}
              </div>
